...
trap cleanup EXIT  # overridden below after TMP_FILES is created
...
trap 'rm -f "$TMP_CONFIG" "$TMP_FILES"' EXIT

git -C "$ROOT" log --name-only --pretty=format: -n "$SORT_BY_CHANGES_MAX_COMMITS" \
  | grep -v '^$' \
  | sort -u \
