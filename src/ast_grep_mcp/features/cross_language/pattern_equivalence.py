...
def _get_suggestions(
    ...
    for rel_id in related:
        if rel_id not in found_patterns and rel_id in PATTERN_DATABASE:
            ...
            suggestions.append(
                f"Related: {rel_id} - {rel_pattern.get('concept', '')}"
            )

    # Suggest category exploration
    if category:
        category_patterns = [
            pid for pid, p in PATTERN_DATABASE.items()
            if p.get("category") == category and pid not in found_patterns
        ]
        if category_patterns[:3]:
            suggestions.append(
                f"More in '{category}': {', '.join(category_patterns[:3])}"
            )

    # Suggest categories to explore
    if not category and len(found_patterns) < 3:
        suggestions.append(
            f"Try searching by category: {', '.join(PATTERN_CATEGORIES[:4])}"
        )
...
def find_language_equivalents_impl(
    ...
    # Get suggestions
    categories = list(set(
        PATTERN_DATABASE.get(pid, {}).get("category")
