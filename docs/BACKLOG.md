...
- [x] **OT-SM-02** (Medium) Decompose `instrumentation.ts` — grew from 377 → **715 lines** (2026-03-11 check). Extracted `evaluation-events.ts` (102 lines) and `span-attributes.ts` (89 lines); main file reduced to 547 lines. (commits fbc394f, 70be56c, review: PASS)
...
- [x] **LM-01** (High) Migrate cache.py to `cachetools.TTLCache` — eliminates 22 LOC manual TTL/LRU tracking. (commit pending, tests: 1770 passed)
- [x] **LM-02** (High) Regex pattern compilation caching — cache compiled patterns via `functools.lru_cache` in `complexity/analyzer.py` and `rewrite/service.py`. Measured: 21.9% speedup on repeated patterns, 1.3x faster. Added `_compile_pattern()` + wrappers (`_findall_cached`, `_search_cached`, `_sub_cached`). Updated 6 call sites. Tests: 1770 passed.
